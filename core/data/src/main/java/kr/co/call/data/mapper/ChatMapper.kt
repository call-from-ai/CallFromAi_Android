 ...
 * [ChatMessagesDTO], [ChatSseMessageDto]를 대응하는 Domain 모델로 변환합니다.
 *
...
object ChatMapper {
    ...
    // SSE로 수신한 실시간 채팅 메시지 DTO를 Domain 메시지 모델로 변환
    fun ChatSseMessageDto.toDomain(): ChatItem.Message = ChatItem.Message(
        chatMessageId = chatMessageId,
        senderType = SenderType.fromApiValue(senderType),
        content = content,
        messageType = MessageType.fromApiValue(messageType),
