...
class CallAudioPlayer @Inject constructor() {
    ...
    // 재생 루프 전용 스코프 - audioTrack.write()가 블로킹 호출이라 CPU 연산용 Default 대신 IO 사용
    ...
    // audioTrack 접근(write/pause/flush/play/stop/release) 직렬화용 락
        ...
        playbackJob = playerScope.launch {
            ...
            for (wav in newQueue) {
                try {
                    ...
                    val (written, trackState, playState) = synchronized(lock) {
                        Triple(
                            audioTrack?.write(pcm, 0, pcm.size), // audioTrack에 쓰기
                            audioTrack?.state,
                    ...
                    Timber.tag(TAG).d(
                        ...
                        frameCount, wav.size, pcm.size, written, trackState, playState,
                    )
